...
class frcnn_client(Node):
                                                                                
    def __init__(self): 
        super().__init__("frcnn_client")
        self.client = self.create_client(Frcnn, '/get_predictions')
        while not self.client.wait_for_service(4.0):
            self.get_logger().info("Waiting for frcnn service ...")
        self.timer = self.create_timer(1.0, self.timer_callback)
        
    def timer_callback(self):
        request = Frcnn.Request()
    ...
    def service_callback(self, future):
        try:
            ...
            self.get_logger().warn("frcnn Service failed: %r" % (e,))

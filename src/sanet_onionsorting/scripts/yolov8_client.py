...
class yolov8_client(Node):
                                                                                
    def __init__(self): 
        super().__init__("yolo_v8_client")
        self.client = self.create_client(Yolo, '/get_predictions')
        while not self.client.wait_for_service(4.0):
            self.get_logger().info("Waiting for YOLO_V8 service ...")
    ...
    def service_callback(self, future):
        try:
            ...
            self.get_logger().warn("YOLO_V8 Service failed: %r" % (e,))
...
def main(args=None):
    ...
    try:
        ...
        print('Yolo_V8 Client Failed: %r' %(e,))

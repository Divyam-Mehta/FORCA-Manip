...
class YOLOv7_DeepSORT:
    ...
    coco_names_path:str ="./io_data/input/classes/coco.names", deepsort = True):
        '''
        args: 
    ...
    def track_video(self,video:str, output:str, skip_frames:int=0, show_live:bool=False, count_objects:bool=False, verbose:int = 0):
        ...
        while True: # while video is running
            ...
            yolo_dets = self.detector.detect(copy(frame), plot_bb = False)  # Get the detections
    ...
    def track_consecutive_frames(self, frame, skip_frames:int=0, show_live:bool=False, count_objects:bool=False, verbose:int = 0):
        '''
        Track objects using two consecutive frames.
        args: 
            video: path to input video or set to 0 for webcam
            output: path to output video
            skip_frames: Skip every nth frame. After saving the video, it'll have very visuals experience due to skipped frames
        ...
        frame = np.frombuffer(frame.data, dtype=np.uint8).reshape(frame.height, frame.width, -1).astype('float32')     # Added by Prasanth Suresh

        if verbose >= 1:start_time = time.time()

        # -----------------------------------------PUT ANY DETECTION MODEL HERE -----------------------------------------------------------------
        if self.deepsort:
            _, yolo_dets = self.detector.detect(copy(frame), plot_bb = False)  # Get the detections

            # print("Yolo dets: ", yolo_dets)
            ...
            for track in self.tracker.tracks:  # update new findings AKA tracks
                ...
                det[f'{bbox[0]}_{idx}'] = [bbox, class_name, track.track_id]
                idx += 1
                color = colors[int(track.track_id) % len(colors)]  # draw bbox on screen
                color = [i * 255 for i in color]
                cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), color, 2)
                ...
                if verbose == 2:
                    print("Tracker ID: {}, Class: {},  BBox Coords (xmin, ymin, xmax, ymax): {}".format(str(track.track_id), class_name, (int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3]))))   
            detections = collections.OrderedDict(sorted(det.items()))
            
            # print("Detections: ", detections)
            # sleep(1000)
        else:
            print("Not using deepsort!")
            [im0, detections] = self.detector.detect(copy(frame), plot_bb = not self.deepsort)  # Get the detections
            frame = copy(im0)
        # -------------------------------- Tracker work ENDS here -----------------------------------------------------------------------
        if verbose >= 1:
            fps = 1.0 / (time.time() - start_time) # calculate frames per second of running detections
        ...
        result = np.asarray(frame)
        # result = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)   # NOTE: Commented out by Prasanth Suresh since YOLO has been trained on BGR space for some bizzare reason!

        save_path = path + '/thirdparty/yolov7_deepsort/inference/output/img.jpg'

                                  ...
                                  double threshold_distance) 
{
    ...
    for (size_t i = 0; i < other_agents.size(); ++i) {
        ...
        if (collision_condition_2)   //  TODO - Need to add condition_1 
        {
            ...
            // Resuce the weightage of the goal velocity if the distance is too small
            double reducing_factor = agent_distance*0.35;

            escape_list.push_back(goal_velocity*reducing_factor + escape_vector*responsibility);
...
//   move_group.startStateMonitor();
  ...
  double epsilon = 0.05;
  double max_len = 0.2;
  double min_len = 0.2;  // 0.05
  double alpha = 0.5;  // 0.35
  ...
  { 
    ...
    while(!robot_goal_reached)   // One whole Pass of PSort
    {
        ...
        robot_distance = std::min({d1, d2, d3});


        RCLCPP_INFO(node->get_logger(), " ");
        RCLCPP_INFO(node->get_logger(), "[%s] Robot Distance   =   %f", robot_id.c_str(), robot_distance);
        RCLCPP_INFO(node->get_logger(), " ");
        ...
        double robot_distance_threshold = 0.3;    // 0.3
        ...
        if(!onion_target_reached)        // Change "if" to "else if" in case you uncomment the Safety Mechanism
        {
            ...
            if (robot_distance > robot_distance_threshold){
                ...
                len = 0.3*distance(current_EE_position, goal_EE_position);
                // len = min_len*robot_distance;
            }
        }
...
//  Disable planning scene 

//  Try recording UR state using rosbag

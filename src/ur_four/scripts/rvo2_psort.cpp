    ...
    const moveit::core::JointModelGroup* joint_model_group)
{
    ...
    for (const auto &onion_id : onion_ids)
    {
        ...
        // if (robot_id == "ur1")
        // {
        //     if (onion_position.x() >= 0.80 || onion_position.y() < -0.35 || onion_position.y() > 0.40)
        //         continue;
        // }
        // else if (robot_id == "ur2")
        // {
        //     if (onion_position.x() <= 0.70 || onion_position.y() < -0.50 || onion_position.y() > 0.40)
        //         continue;
        // }
...
// This function returns the suggested velocity from RVO2
  ...
  const std::string & robot_id)
{
  ...
  {
    // Return preferred velocity directly
    RCLCPP_INFO(logger, "[%s] RVO2 INACTIVE: pref_vel = (%.2f, %.2f), distance = %.3f", robot_id.c_str(), pref_vel.x(), pref_vel.y(), distance);
    return Eigen::Vector3d(pref_vel.x(), pref_vel.y(), 0.0);
  }
}
...
//   move_group.startStateMonitor();
  ...
  { 
    ...
    while(!robot_goal_reached)   // One whole Pass of PSort
    {
        ...
        // Plan for those Goal Position using RVO2
        ...
        // if (robot_id == "ur1") 
        // {
        //     Eigen::Vector3d ur1_dist = current_EE_position - ur1_prev_EE_position;
        //     Eigen::Vector3d ur2_dist = other_robot_current_EE_position - ur2_prev_EE_position;

        //     if (ur1_dist.norm() < 0.001)
        //     {
        ...
        //     else
        //     {
        //         current_EE_vel = ur1_dist*ur1_speed/ur1_dist.norm();
        //     }


        //     if (ur2_dist.norm() < 0.001)
        //     {
        //         other_robot_current_EE_vel = ur2_dist;
        //     }

        //     else
        //     {
        //         other_robot_current_EE_vel = ur2_dist*ur2_speed/ur2_dist.norm();
        //     }
        // } 
        ...
        // else 
        // {
        //     Eigen::Vector3d ur2_dist = current_EE_position - ur2_prev_EE_position;
        //     Eigen::Vector3d ur1_dist = other_robot_current_EE_position - ur1_prev_EE_position;
        ...
        //     else
        //     {
        //         current_EE_vel = ur2_dist*ur2_speed/ur2_dist.norm();
        //     }


        //     if (ur1_dist.norm() < 0.001)
        //     {
        //         other_robot_current_EE_vel = ur1_dist;
        //     }

        //     else
        //     {
        //         other_robot_current_EE_vel = ur1_dist*ur1_speed/ur1_dist.norm();
        //     }
        ...
        if (next_EE_vel.norm() < 0.01)
        {
            target_unit_vec = Eigen::Vector3d(0.0, 0.0, 0.0);
        }
...
int main(int argc, char * argv[])
{
  ...
  auto node = rclcpp::Node::make_shared("rvo2_psort");

                                  ...
                                  double threshold_distance) 
{
    ...
    for (size_t i = 0; i < other_agents.size(); ++i) {
        ...
        if (collision_condition_2)   //  TODO - Need to add condition_1 
        {
            ...
            escape_list.push_back(goal_velocity*0.0 + escape_vector*responsibility);
    ...
    if (escape_list.empty()) {
        ...
        std::cout << std::string(100, '-') << std::endl;
        std::cout << "Non - Collision Case (However, Robots are close to each other)" << std::endl;
        std::cout << std::string(100, '-') << std::endl;
...
//   move_group.startStateMonitor();
  ...
  { 
    ...
    while(!robot_goal_reached)   // One whole Pass of PSort
    {
        ...
        double robot_distance_threshold = 0.5;
        ...
        if (robot_id == "ur1") 
        {
            ...
            else
            {
                ...
                next_EE_vel = ORCA(node, main_agent, {other_agent}, goal_EE_position, current_EE_position, pref_speed, robot_id, robot_distance_threshold)[0];
...
int main(int argc, char * argv[])
{
  ...
  auto node = rclcpp::Node::make_shared("multi_thread_psort_orca");
...
//   std::vector<double> ur1_inspect_pos = {0.386, -0.4872, 1.0, 3.14, 0, 3.14};
//   std::vector<double> ur2_inspect_pos = {1.1171, -1.0, 1.0, 3.14, 0, 3.14};
//   std::vector<double> ur3_inspect_pos = {0.3651, 1.1075, 1.0, 3.14, 0, 3.14};
//   std::vector<double> ur4_inspect_pos = {1.04, 0.63, 1.0, 3.14, 0, 3.14};

//   std::vector<double> ur1_inspect_approach = {0.386, -0.4872, 1.0, -1.5788, 0.057, 2.6136};
//   std::vector<double> ur2_inspect_approach = {1.1171, -1.0, 1.0, -1.5588, 0.0194, -2.5824};
//   std::vector<double> ur3_inspect_approach = {0.3651, 1.1075, 1.0, 1.588, 0.0147, -2.6738};
//   std::vector<double> ur4_inspect_approach = {1.04, 0.63, 1.0, 1.61, 0, 3.14};
  ...
  std::thread ur1_thread(runRobot, node, ur1_planning_group, ur1_joint_model_group, "ur1", ur1_action_name, ur1_ee, ur1_other_ee, ur1_gz_poses, std::ref(ur1_move_group), ur1_other_move_groups);
  std::thread ur2_thread(runRobot, node, ur2_planning_group, ur2_joint_model_group, "ur2", ur2_action_name, ur2_ee, ur2_other_ee, ur2_gz_poses, std::ref(ur2_move_group), ur2_other_move_groups);
  std::thread ur3_thread(runRobot, node, ur3_planning_group, ur3_joint_model_group, "ur3", ur3_action_name, ur3_ee, ur3_other_ee, ur3_gz_poses, std::ref(ur3_move_group), ur3_other_move_groups);
  std::thread ur4_thread(runRobot, node, ur4_planning_group, ur4_joint_model_group, "ur4", ur4_action_name, ur4_ee, ur4_other_ee, ur4_gz_poses, std::ref(ur4_move_group), ur4_other_move_groups);
...
// Solve the IK soln of all the onions on the fly (choose onion first and then solve for the IK)
